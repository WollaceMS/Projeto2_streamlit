...
def criar_card(icone, numero, texto, coluna_card):
    ...
    coluna_esquerda, coluna_direita = container.columns([1, 2.5]) #dentro do container add as colunas
    coluna_esquerda.image(f'imagens/imagens/{icone}')
...
base_status = base.groupby('Status', as_index=False).count() # Coluna não pode ser indice, se não o gráfico 'não entende'
...
st.plotly_chart(grafico) #mesma ideia do plt.show()